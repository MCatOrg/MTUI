<template>
  <div :class="[className]">
    <div
      class="mtui-mask"
      @click="close(closeOnClickModal)"
      v-show="currentValue"
    ></div>
    <transition name="actionsheet-float">
      <div class="mtui-actionsheet" v-show="currentValue">
        <div
          class="mtui-actionsheet__title"
          v-if="!!title"
        ...
        <div class="mtui-actionsheet__menu">
          <div
            class="mtui-actionsheet__cell"
            :class="{ 'mtui-actionsheet__active': currentIndex == i }"
            v-for="(menu, i) in menus"
            :key="i"
            @click.stop="itemClick(menu, i)"
            :style="{ color: menu.color ? menu.color : 'initial' }"
          >
            {{ menu.text }}
          </div>
        </div>
