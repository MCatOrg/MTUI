    ...
    <div :class="['mtui-steps',orient,className]">
        ...
        <template v-if="orient === 'horizon'">
            <template v-for="(value,index) in computedLists">
                <div :class="['mtui-step',value.status?'active':'']" :style="value.status ? computedTextColor:''" :key="index">
                    <div class="mtui-step_chart">
                        <span :class="['step-chart_dot',value.animate?'cur':'']" :style="value.status ? computedColor:''">
                            <span :class="['step-chart_animate',value.animate?animate:'']" :style="value.status ? computedColor:''"></span>
                        </span>
                        <span class="step-chart_line"></span>
                        <span :class="['step-chart_line',value.status?'cur':'']" :style="value.status ? computedColor:''"></span>
                    </div>
            ...
            <div v-for="(value,index) in computedLists" :key="index" :class="['mtui-step',value.status?'active':'']">
                <div class="mtui-step_chart">
                    <span class="step-chart_dot" :style="value.status ? computedColor:''">
                        <span :class="['step-chart_animate',value.animate?animate:'']" :style="value.animate ? computedColor:''"></span>
                    </span>
                    <!-- <span :class="['step-chart_line',value.animate?'lastActive':'']"></span> -->

                    <span class="step-chart_line"></span>
                    <span :class="['step-chart_line',value.status&&!value.animate?'cur':'']" :style="value.status&&!value.animate ? computedColor:''"></span>
                </div>
